...
def main():
    ...
    if "input_enabled" not in st.session_state:
        st.session_state.input_enabled = False  # Désactivé au départ

    st.header("Chat with multiple PDFs :books:")

    # Désactivé par défaut et activé après traitement des PDFs
    user_question = st.text_input("Ask a question about your documents:", disabled=not st.session_state.input_enabled)
        ...
        if st.button("Process"):
            with st.spinner("Processing"):
                ...
                if not pdf_paths:
                    ...
                    # Active le champ de texte après traitement
